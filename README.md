...
- `Tk`, `Toplevel`, `Button`, `Checkbutton`, `Frame`, `Label`, `Menu`, `Menubutton`, `Radiobutton`, `Scale`, `OptionMenu` and `LabelFrame` are fully implemented. However, a lot of their functionality is inherited from `Misc` and therefor not implemented as of yet.
