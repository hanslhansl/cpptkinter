...
- `Tk`, `Toplevel`, `Button`, `Frame`, `Label`, `Menubutton`, `Scale` and `LabelFrame` are fully implemented. However, a lot of their functionality is inherited from `Misc` and therefor not implemented as of yet.
- `Menu` is partially implemtented.
- `_tkinter`is implemented for the most part and available in `namespace cpptkinter::_cpptkinter`

The next step is to fully implement `Misc`, `Image`, `PhotoImage` and `BitmapImage`.
